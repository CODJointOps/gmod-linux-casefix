...
#if defined(GSDK_SAFETYHOOK_LOCKLESS_ORIGINALS)
        return unsafe_ccall<RetT>(args...);
#else
        std::scoped_lock lock{m_mutex};
...
#if defined(GSDK_SAFETYHOOK_LOCKLESS_ORIGINALS)
        return unsafe_thiscall<RetT>(args...);
#else
        std::scoped_lock lock{m_mutex};
...
#if defined(GSDK_SAFETYHOOK_LOCKLESS_ORIGINALS)
        return unsafe_stdcall<RetT>(args...);
#else
        std::scoped_lock lock{m_mutex};
...
#if defined(GSDK_SAFETYHOOK_LOCKLESS_ORIGINALS)
        return unsafe_fastcall<RetT>(args...);
#else
        std::scoped_lock lock{m_mutex};
