...
std::expected<void, InlineHook::Error> InlineHook::e9_hook(const std::shared_ptr<Allocator>& allocator) {
    ...
    for (auto ip = m_target, tramp_ip = m_trampoline.data(); ip < m_target + m_original_bytes.size(); ip += ix.length) {
        ...
        if (is_relative && ix.raw.disp.size == 32) {
            ...
            // i386 PIC prologue special-case. The Linux/clang i386 PIC ABI
            // emits a `call __x86.get_pc_thunk.<reg>` at the head of every
            // PIC function. The thunk is a 4-byte sequence
            // `mov (%esp), %<reg>; ret` which gives the caller the
            // address of the instruction immediately following the call;
            // the caller then does `addl $_GLOBAL_OFFSET_TABLE_, %<reg>`
            // (a relocation patched at load time) to materialize the
            // module's GOT base.
            //
            // If we naively relocate the `call` instruction into the
            // trampoline, the return-address pushed by the CPU is now
            // inside the trampoline arena, not the original module's
            // .text — so the subsequent `addl $_GOT_OFFSET_, %<reg>`
            // resolves the GOT base relative to the trampoline. Every
            // PIC-relative deref in the rest of the relocated prologue
            // (and, after we jmp back to original_fn+N, in the original
            // function's body too) reads garbage. Live bts confirmed
            // this on SDL_PollEvent and steamoverlay vkQueuePresentKHR.
            //
            // Replace `e8 <rel32>` (call rel32) with
            // `b8+reg <imm32>` (mov $imm32, %<reg>) — also 5 bytes —
            // where imm32 = original next-pc. The CPU never enters the
            // thunk; %<reg> is materialized directly, and the original
            // module's `addl $_GOT_OFFSET_, %<reg>` runs unchanged.
            //
            // The thunks vary only in destination register; their body
            // is always `8b XX 24 c3` (4 bytes: mov [esp], <reg>; ret).
            if (ix.opcode == 0xE8 && ix.length == 5) {
                const auto* tgt = reinterpret_cast<const uint8_t*>(target_address);
                // Read 4 bytes of the alleged thunk best-effort. If the
                // target is unreadable, fall through to the regular
                // relocate path (which would also fail, but consistently).
                uint8_t t0 = 0, t1 = 0, t2 = 0, t3 = 0;
                bool readable = true;
                try {
                    t0 = tgt[0]; t1 = tgt[1]; t2 = tgt[2]; t3 = tgt[3];
                } catch (...) { readable = false; }
                if (readable && t0 == 0x8B && t2 == 0x24 && t3 == 0xC3) {
                    // ModR/M byte: mod=00 (mem [esp]), reg=<dst>, rm=100 (SIB).
                    const uint8_t reg_idx = static_cast<uint8_t>((t1 >> 3) & 0x7);
                    const uint32_t imm = static_cast<uint32_t>(
                        reinterpret_cast<uintptr_t>(ip + ix.length));
                    tramp_ip[0] = static_cast<uint8_t>(0xB8 | reg_idx);
                    store(tramp_ip + 1, imm);
                    tramp_ip += 5;
                    continue;
                }
            }
...
public:
    ...
    // Lock-free fast-path bookkeeping for trap_handler.  This VEH runs on
    // *every* access violation in the host process, and Source-family games
    // (gmod x64, tf2, css) fire many SEH-protected pointer-validation AVs
    // per frame from inside engine/client code.  Acquiring TrapManager::mutex
    // for each one serialized hundreds of cross-thread AVs on a single
    // process-global mutex, which the kernel resolves via NtWaitForKey-
    // edObject + scheduler wake -- this lit up as 70 %+ ntoskrnl.exe leaf
    // time in WPR captures and ~60 fps lost just from injecting dzsdk.
    //
    // The slot array is a sparse, lock-free set of [start, end) ranges
    // covering currently-armed traps PLUS the 4 KB page surrounding each
    // (to subsume the slow path's find_trap_page).  add_trap publishes
    // through the slot; remove_trap clears it.  trap_handler reads slots
    // lock-free and only descends into the mutex+map path if the faulting
    // address overlaps an armed range.
    static constexpr size_t MAX_HOT_TRAPS = 512;
    static std::array<std::atomic<uintptr_t>, MAX_HOT_TRAPS> hot_starts;
    static std::array<std::atomic<uintptr_t>, MAX_HOT_TRAPS> hot_ends;
    // If we ever fail to publish a trap into the hot table (table full),
    // we MUST fall back to always taking the slow path -- otherwise an AV
    // in that unpublished trap region would be missed by the fast path
    // and the thread would crash instead of being redirected by fix_ip.
    // Hook install correctness > VEH fast-path perf.
    static std::atomic<bool> hot_overflow;
    ...
    void add_trap(uint8_t* from, uint8_t* to, size_t len) {
        ...
        // Publish into the lock-free hot-range table.  Two slots per trap so
        // the fast path covers the from-page and the to-page (matches the
        // slow path's find_trap_page semantics).  CAS into the first free
        // pair we find; if the table is full we silently fall through and
        // simply pay the mutex cost for AVs in this trap (still correct,
        // just slower for that one).
        const auto publish = [](uintptr_t start, uintptr_t end) {
            for (size_t i = 0; i < MAX_HOT_TRAPS; ++i) {
                uintptr_t expected = 0;
                if (hot_starts[i].compare_exchange_strong(expected, start,
                        std::memory_order_release, std::memory_order_relaxed)) {
                    hot_ends[i].store(end, std::memory_order_release);
                    return;
                }
            }
            // Out of slots.  Mark overflow so the VEH always takes the
            // mutex-guarded slow path from here on -- otherwise this trap
            // would be invisible to the fast path and threads AV-ing in
            // its range would crash.
            hot_overflow.store(true, std::memory_order_release);
        };
        publish(reinterpret_cast<uintptr_t>(info.from_page_start),
                reinterpret_cast<uintptr_t>(info.from_page_end));
        if (info.to_page_start != info.from_page_start ||
            info.to_page_end != info.from_page_end) {
            publish(reinterpret_cast<uintptr_t>(info.to_page_start),
                    reinterpret_cast<uintptr_t>(info.to_page_end));
        }
    ...
    void remove_trap(uint8_t* from) {
        auto it = m_traps.find(from);
        if (it == m_traps.end()) {
            return;
        }
        const auto unpublish = [](uintptr_t start) {
            for (size_t i = 0; i < MAX_HOT_TRAPS; ++i) {
                if (hot_starts[i].load(std::memory_order_acquire) == start) {
                    hot_ends[i].store(0, std::memory_order_relaxed);
                    hot_starts[i].store(0, std::memory_order_release);
                    return;
                }
            }
        };
        unpublish(reinterpret_cast<uintptr_t>(it->second.from_page_start));
        if (it->second.to_page_start != it->second.from_page_start) {
            unpublish(reinterpret_cast<uintptr_t>(it->second.to_page_start));
        }
        m_traps.erase(it);
    }

    static bool hot_contains(uintptr_t addr) {
        // If we've ever overflowed the slot table, we can't trust the
        // fast path -- some trap is unrepresented.  Always slow-path.
        if (hot_overflow.load(std::memory_order_acquire)) {
            return true;
        }
        for (size_t i = 0; i < MAX_HOT_TRAPS; ++i) {
            const uintptr_t end = hot_ends[i].load(std::memory_order_acquire);
            if (end == 0) {
                continue;
            }
            const uintptr_t start = hot_starts[i].load(std::memory_order_acquire);
...
private:
    ...
    static LONG CALLBACK trap_handler(PEXCEPTION_POINTERS exp) {
        ...
        const uintptr_t addr = exp->ExceptionRecord->ExceptionInformation[1];

        // Lock-free fast path: 99 %+ of AVs in a hooked Source-family
        // process are unrelated to our trap regions, so bail before
        // serializing on the global mutex.  See the long comment on
        // hot_starts/hot_ends above.
        if (!hot_contains(addr)) {
            return EXCEPTION_CONTINUE_SEARCH;
        }

        std::scoped_lock lock{mutex};
        auto* faulting_address = reinterpret_cast<uint8_t*>(addr);
        auto* trap = instance ? instance->find_trap(faulting_address) : nullptr;

        if (trap == nullptr) {
            if (instance && instance->find_trap_page(faulting_address) != nullptr) {
...
std::array<std::atomic<uintptr_t>, TrapManager::MAX_HOT_TRAPS> TrapManager::hot_starts{};
std::array<std::atomic<uintptr_t>, TrapManager::MAX_HOT_TRAPS> TrapManager::hot_ends{};
std::atomic<bool> TrapManager::hot_overflow{false};
